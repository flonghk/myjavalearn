...
public static void main(String[] args) throws Exception, IOException {
	String excelPath = "E:/API/APIExcelDemo/APIMergeDemo.xlsx";
	List caseExeclsList = new ArrayList<>();
	ReadMergeRegionExcel mergeRegionExcel = new ReadMergeRegionExcel();
	excelReader = new ExcelReader(excelPath);
	
	String excelName = excelPath.substring(excelPath.lastIndexOf("/")+1,excelPath.lastIndexOf("."));
	for (int sheetNumber = 0; sheetNumber < excelReader
			.getSheetCount(); sheetNumber++) {
	
		Sheet sheet = excelReader.wb.getSheetAt(0);
		for (int i = 1; i <=excelReader.getRowCount(); i++) {
		//	System.out.println("************************");
			for (int j = 1; j <=excelReader.getColumnCount(i); j++) {
				boolean isMerge = ReadMergeRegionExcel.isMergedRegion(sheet,i-1,j-1);
				if(isMerge){
				String rs = ReadMergeRegionExcel.getMergedRegionValue(sheet, i-1,j-1);
				System.out.print(rs+" ");}
					else System.out.print(excelReader.getCellValue(i, j)+" ");
			}
			System.out.println("");
		}
		}
